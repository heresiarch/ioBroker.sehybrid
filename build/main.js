...
var import_config_validation = require("./lib/config-validation");
var import_modbus_client = require("./lib/modbus-client");
var import_state_manager = require("./lib/state-manager");
var import_storedge_control_map = require("./lib/storedge-control-map");
var import_sunspec_decode = require("./lib/sunspec-decode");
var import_sunspec_map = require("./lib/sunspec-map");
var import_sunspec_reader = require("./lib/sunspec-reader");
const MAX_CONSECUTIVE_FAILURES = 10;
const STOREDGE_BLOCK_SPAN_A_BASE = 57348;
const STOREDGE_BLOCK_SPAN_B_BASE = 57357;
function wordsForStorEdgeDef(def, spanA, spanB) {
  if (def.address >= STOREDGE_BLOCK_SPAN_B_BASE) {
    const offset2 = def.address - STOREDGE_BLOCK_SPAN_B_BASE;
    return spanB.slice(offset2, offset2 + def.length);
  }
  const offset = def.address - STOREDGE_BLOCK_SPAN_A_BASE;
  return spanA.slice(offset, offset + def.length);
}
function storEdgeWireDatatype(def) {
  if (def.kind === "uint16") {
    return "uint16";
  }
  if (def.kind === "float32") {
...
class Sehybrid extends utils.Adapter {
  ...
  async onReady() {
    ...
    const result = (0, import_config_validation.validateConfig)(this.config);
    if (!result.valid) {
      ...
      const messages = Object.entries(result.errors).map(([field, msg]) => `${field}: ${msg}`).join("; ");
    ...
    this.reader = new import_sunspec_reader.SunSpecReader(logger);
    this.stateManager = new import_state_manager.StateManager(this);
    this.modbusClient = new import_modbus_client.ModbusClient();
   ...
   */
  async handleStorEdgeControlChange(id, state) {
    const prefix = `${this.namespace}.StorEdgeControlBlock.`;
    if (!id.startsWith(prefix)) {
      return;
    }
    const leaf = id.slice(prefix.length);
    const def = (0, import_storedge_control_map.findStorEdgeControlDef)(leaf);
    if (!def) {
      return;
    }
    ...
    try {
      if (def.kind === "uint16") {
        ...
        await this.modbusClient.writeMultipleRegisters(def.address, (0, import_sunspec_decode.encodeFloat32le)(value));
      } else {
        await this.modbusClient.writeMultipleRegisters(def.address, (0, import_sunspec_decode.encodeUint32le)(value));
      }
  ...
  async pollOnce() {
    ...
    try {
      if (!client.isConnected()) {
        ...
        await client.connect(this.config.host, this.config.port, this.config.unitId, import_modbus_client.DEFAULT_TIMEOUT_MS);
      }
      ...
      for (const meter of meters) {
        const channel = `meter.${meter.index}`;
      ...
      for (const battery of batteries) {
        const channel = `battery.${battery.index}`;
        await stateManager.ensureChannel(channel);
      ...
      const blockWordsA = await client.readHoldingRegisters(57348, 9);
      const blockWordsB = await client.readHoldingRegisters(57357, 5);
      for (const def of import_storedge_control_map.STOREDGE_CONTROL_REGISTERS) {
        ...
        const value = (0, import_sunspec_decode.decodeRegisters)(words, storEdgeWireDatatype(def));
        if (value !== null) {
          await stateManager.writeStorEdgeValue(def, value);
        }
      }
  ...
  async handleTestConnection(obj) {
    ...
    const result = (0, import_config_validation.validateConfig)({ host, port, unitId, pollInterval: 30 });
    ...
    const client = new import_modbus_client.ModbusClient();
    try {
      await client.connect(host, port, unitId, import_modbus_client.DEFAULT_TIMEOUT_MS);
      const idWords = await client.readHoldingRegisters(import_sunspec_map.COMMON_BASE, 2);
      const sunsId = (0, import_sunspec_decode.decodeRegisters)(idWords, "uint32");
      if (sunsId !== 1400204883) {
        reply({
          result: "failure",
      ...
      const manuWords = await client.readHoldingRegisters(import_sunspec_map.COMMON_BASE + 4, 16);
      const modelWords = await client.readHoldingRegisters(import_sunspec_map.COMMON_BASE + 20, 16);
      const manufacturer = (0, import_sunspec_decode.decodeRegisters)(manuWords, "string");
      const model = (0, import_sunspec_decode.decodeRegisters)(modelWords, "string");
      reply({
        result: "success",
...
if (require.main !== module) {
  module.exports = (options) => new Sehybrid(options);
} else {
  (() => new Sehybrid())();
}
